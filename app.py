...
def get_complaints():
    try:
        with pyodbc.connect(conn_str) as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT id, student_name, email, title, description, type, file_url, status, created_at
                FROM Complaints
            ...
            for row in rows:
                complaints.append({
                    ...
                    "status": row.status,
                    "submitted_at": row.created_at.strftime('%Y-%m-%d %H:%M:%S') if row.created_at else "N/A"
                })
